...
<body>
	...
	<div class="container mt-5">
		<div class="row mt-3">
			<div class="col text-center fw-bold fs-1" >자유게시판 신고</div>
		</div>
				...
				<table class="table table-hover">
					...
					<tbody>
						<c:forEach items="${resultList }" var="data">
							<tr>
								<td style="width: 10%" class="text-center">${data.reportBfVo.rbf_no }</td>
								<td onclick="location.href='../board/freeboard_read_page.do?bf_no=${data.bfvo.bf_no}'" style="cursor:pointer; width: 40%" class="text-center">${data.reportBfVo.rbf_content }</td>
								<td style="width: 10%" class="text-center">${data.memberVo.m_nick }</td>
								<td style="width: 10%" class="text-center">
									<fmt:formatDate pattern="yyyy.MM.dd." value="${data.reportBfVo.rbf_regdate }"/>
								</td>
								<c:if test="${data.confirm ==0 }">
									<td style="width: 10%" class="text-center">
										<a href="${pageContext.request.contextPath}/manager/free_penalty_process.do?bf_no=${data.bfvo.bf_no}"><input class="btn btn-danger" type="button" value="제재"></a>	
									</td>
								</c:if>
								<c:if test="${data.confirm !=0 }">
									<td style="width: 10%" class="text-center">
