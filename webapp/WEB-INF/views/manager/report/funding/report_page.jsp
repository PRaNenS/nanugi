...
<body>
	...
	<div class="container mt-5">
		<div class="row mt-3">
			<div class="col text-center fw-bold fs-1" >펀딩 후기글 신고</div>
		</div>
				...
				<table class="table table-hover">
					...
					<tbody>
						<c:forEach items="${resultList }" var="data">
							<tr>
								<td style="width: 10%" class="text-center">${data.reportRFVo.rerf_no }</td>
								<td onclick="location.href='../review_funding/read_page.do?rf_no=${data.reviewFundingVo.rf_no }'" style="cursor:pointer; width: 40%" class="text-center">${data.reportRFVo.rerf_content }</td>
								<td style="width: 10%" class="text-center">${data.memberVo.m_nick }</td>
								<td style="width: 10%" class="text-center">
									<fmt:formatDate pattern="yyyy.MM.dd." value="${data.reportRFVo.rerf_date}"/>
								</td>
								<c:if test="${data.confirm ==0 }">
									<td style="width: 10%" class="text-center">
										<a href="${pageContext.request.contextPath}/manager/fun_penalty_process.do?rf_no=${data.reviewFundingVo.rf_no }"><input class="btn btn-danger" type="button" value="제재"></a>	
									</td>	
								</c:if>
								<c:if test="${data.confirm !=0 }">
									<td style="width: 10%" class="text-center">
